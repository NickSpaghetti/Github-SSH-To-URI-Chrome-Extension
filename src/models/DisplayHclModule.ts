...
export class DisplayHlcModule implements IDisplayHlcModule {
    ...
    public static async buildAsync(
        ...
        terraformFetchService: ITerraformFetchService,
    ): Promise<DisplayHlcModule> {
        const hclSourceService: HclSourceService = new HclSourceService(terraformFetchService);
        const instance = new DisplayHlcModule(uri, terraformModule);
        if (
            terraformModule.provider.source !== undefined &&
            terraformModule.provider.source !== "" &&
            terraformModule.provider.version !== undefined
        ) {
            ...
            instance.modifiedSourceType = await hclSourceService.resolveSourceAsync(
                instance.sourceType,
                terraformModule.provider.source,
                terraformModule.moduleName,
                terraformModule.provider.version,
                new URL(uri),
            );
        }
        return instance;

...
export class TerraformFetchService {
     ...
     */
    public async getTerraformVersionsAsync(
        providerSource: string,
        providerType: string,
    ): Promise<Array<string>> {
        const response = await this.terraformDataAccess.getTerraformVersionsAsync(
            providerSource,
            providerType,
        );
        if (!response.ok) {
     ...
     */
    public async verifyTerraformVersionAsync(
        providerSource: string,
        providerType: string,
        version: string,
    ): Promise<boolean> {
        try {
            const response = await this.terraformDataAccess.verifyTerraformVersionAsync(
                providerSource,
                providerType,
                version,
            );

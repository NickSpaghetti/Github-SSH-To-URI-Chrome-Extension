...
export class HclService {
    ...
    findTerraformSources = (hclFile: IHclFile[]): Nullable<Map<string, TerraformModule>> => {
        if (
            hclFile[0] === null ||
            hclFile[0]?.terraform === undefined ||
            hclFile[0]?.terraform[0] === undefined
        ) {
            console.log("No terraform section found");
            return null;
        }

        const foundModules: Map<string, TerraformModule> = new Map<string, TerraformModule>();
        try {
            hclFile[0]?.terraform.forEach((tf) => {
                ...
                if (tf.required_providers !== undefined) {
                    const requiredProvidersCount: number = 0;
                    const providers = tf.required_providers[requiredProvidersCount] ?? [];
                    Object.keys(providers).forEach((key) => {
                        const providerType = providers[
                            key as keyof RequiredProvider
                        ] as ProviderType;
    ...
    findModuleSources = (hclFile: IHclFile[]): Nullable<Map<string, TerraformModule>> => {
        ...
        try {
            ...
            for (const [moduleName, modules] of Object.entries(hclFile[0].module)) {
    ...
    findSourcesAsync = async (): Promise<DisplayHlcModule[]> => {
        const dataTargetElement = document.getElementById(
        ...
        const sources: DisplayHlcModule[] = [];
        try {
            const hclFile = hcl.parseToObject(innerText) as IHclFile[];
            const terraformSources = this.findTerraformSources(hclFile);
            const moduleSources = this.findModuleSources(hclFile);
            ...
            for (const [_, module] of mergedSources) {
                if (module.provider.source !== undefined) {
                    sources.push(
                        await DisplayHlcModule.buildAsync(
                            window.location.href,
                            module,
                            this.terraformFetchService,
                        ),
                    );
                }
            }
